...
- A threshold whose metric is absent from the results now **fails** rather than passing. `pywrkr.reporting.evaluate_thresholds` substitutes `0.0` for a metric it cannot find, which makes `p95 < 500ms` green on a run that never got a response; the file-based path used by CI refuses to do that. (#192)
